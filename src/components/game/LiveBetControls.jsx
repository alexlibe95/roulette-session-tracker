...
export const LiveBetControls = forwardRef(function LiveBetControls(
  ...
  const panelId = 'live-bet-panel';

  return (
    <div className={`live-bet-controls${expanded ? ' is-open' : ''}`}>
      <button
        type="button"
        className="live-bet-toggle"
        aria-expanded={expanded}
        aria-controls={panelId}
        onClick={() => setExpanded((open) => !open)}
      >
        <span className="live-bet-toggle-copy">
          <span className="live-bet-label">Stake this spin</span>
          <strong className={`live-bet-toggle-amount${overBet ? ' is-over' : ''}`}>
            ${formatMoney(currentBet)}
          </strong>
        </span>
        <ChevronDown className="live-bet-chevron" size={22} aria-hidden />
      </button>

      {expanded && (
        <div id={panelId} className="live-bet-panel">
          <div className="live-bet-main">
            <label htmlFor="live-bet-input" className="sr-only">
              Stake this spin
            </label>
